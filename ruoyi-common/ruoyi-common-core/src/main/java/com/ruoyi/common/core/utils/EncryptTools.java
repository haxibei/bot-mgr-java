...
public final class EncryptTools {
    ...
    public static byte[] encryptByBase64(byte[] data, byte[] key) {
        ...
        try {
            ...
            log.warn("加密参数有误：加密字符串为：{}， clientId：{}", e, new String(data), new String(key));
        }
    ...
    public static byte[] decryptByBase64(byte[] data, byte[] key) {
        ...
        try {
            ...
            log.warn("解密参数有误：加密字符串为：{}， clientId：{}", e, new String(data), new String(key));
        }
        return result;
    }
    ...
    public static byte[] decryptByAES(byte[] content, byte[] keyBytes) {
        ...
        try {
            ...
            //e.printStackTrace(); 
            log.debug("decryptByAES error:{}", e.getMessage());
        }
     ...
     * @return
     */
    public static boolean hasBeenEncryptByAES(String data, String key) {
        try {
            EncryptTools.decryptByAES(data, key);
            return false;
        } catch (Exception e) {
            return true;
        }
