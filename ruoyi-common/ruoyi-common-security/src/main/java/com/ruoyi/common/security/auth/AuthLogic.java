...
public class AuthLogic
{
    ...
    /**
     * 根据注解(@BaseRequiresPermissions)鉴权, 如果验证未通过，则抛出异常: NotPermissionException
     *
     * @param requiresPermissions 注解对象
     */
    public void checkBasePermi(String module, BaseRequiresPermissions requiresPermissions)
    {
        String[] newPerm = getNewPerm(module, requiresPermissions.value());
        SecurityContextHolder.setPermission(StringUtils.join(newPerm, ","));
        if (requiresPermissions.logical() == Logical.AND)
        {
            checkPermiAnd(newPerm);
        }
        else
        {
            checkPermiOr(newPerm);
        }
    }

    private String[] getNewPerm(String module, String[] strs) {
